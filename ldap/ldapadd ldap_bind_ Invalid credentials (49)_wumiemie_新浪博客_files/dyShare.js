;(function($){
    ...
    function _open(cfg,typ,context){
        ...
        if(typ==="quan"){
            ...
            if (/iphone|ipod|ipad|ios|android|BlackBerry|Windows Phone|Nokia/gi.test(UA)) {
                var params = {
                    "title" : decodeURIComponent(stitle), // 标题
                    "url" : decodeURIComponent(surl), // 网页url
                    "imageUrl" : decodeURIComponent(spic) // 配图url
                    // "source" : "", // 来源
                }
                params = encodeURIComponent(JSON.stringify(params));
                if (!!UA.match(/MicroMessenger| qq\//i)) {
                    // if(/MicroMessenger/i.test(UA) && /Android/gi.test(UA) ){
                    //     location.href = location.protocol + '//a.app.qq.com/o/simple.jsp?pkgname=com.duoyi.implayer';
                    // }else{
                        location.href = location.protocol + '//zmweb.duoyi.com/download_app/index.html?params=' + params;
                    //}
                } else {
                    var nowTime = Date.now();
                    var checkTime = 1000;
                    window.location.href = "duoyizhanmeng://web/shareWebsiteToZone?params=" + params;
                    // $(document).on('visibilitychange', function() {
                        
                    // })
                    window.setTimeout(function(){
                        if (Date.now() - nowTime - 100 < checkTime && UA.indexOf('zhanmengapp') === -1) {
                            window.location.href = "https://dygame.duoyi.com/m/?from=dyshare";
                        }
                    }, checkTime)
                }
            } else {
                window.open("https://quan.duoyi.com/dyq/api/getShareMessage?url=" + surl + "&title=" + stitle + "&picture_url=" + spic + "&description=" + sdesc);
            }
        }
        ...
        if(typ==="weixin"){
            window.open("https://www.duoyi.com/weichat/code.shtm?url="+surl+"&title="+stitle+"&desc="+sdesc,"_blank","width= 620,height= 700,top= 100, left= 200, toolbar=no, menubar= no, scrollbars= no, resizable= yes, location= no, status=no ");
        }
