...
function _checksec(type, idhash, showmsg, recall, modid) {
	...
	x.loading = '';
	$('checksec' + type + 'verify_' + idhash).innerHTML = '<img id="checksec_loading_' + idhash + '" src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" />';
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			obj.innerHTML = '<img id="checksec_right_' + idhash + '" src="'+ IMGDIR + '/check_right.gif" width="16" height="16" class="vm" />';
			if(showmsg) {
				recall(1);
			}
		} else {
			obj.innerHTML = '<img id="checksec_error_' + idhash + '" src="'+ IMGDIR + '/check_error.gif" width="16" height="16" class="vm" />';
			if(showmsg) {
				if(type == 'code') {
