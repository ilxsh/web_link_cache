...
function fastpostvalidate(theform, noajaxpost) {
    ...
    var label = $('fastpostsubmit').childNodes[0];
    label.innerText = '正在发表';
...
function checkForumnew(fid, lasttime) {
    ...
    x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
        if(s > 0) {
            ...
            var msg = "有新回复的主题，点击查看";
            if(LANG == 'en'){
                msg = 'There are new thread replies, click for View';
            }
            var colspan = table.getElementsByTagName('tbody')[0].rows[0].children.length;
            var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">'+msg, 'colspan': colspan }}};
