(function(window, undefined){
	...
	$.ui.Scroll = function(body, axis, handle) {
		...
		t._bCnt = body.parent();
		t._bPos = 0;
		t._bRange = Math.max(0, t._body.width() - t._bCnt.width());
			
		if(handle !== undefined){
			t._handle = handle;
			t._hCnt = handle.parent();
			t._hPos = 0;
			t._hRange = Math.max(0, t._hCnt.width() - t._handle.width());
			...
			jQuery(t._hCnt).click(t._mouseClick.bind(t));
		}
	};
	...
	$.ui.Scroll.prototype = {
		...
		_setPos: function(pos){
			...
			t._bCnt.scrollLeft(t._bPos);
			
			if(t._handle){
				t._hPos = t.bhRate ? pos / t.bhRate : 0;
				//console.log(t._hPos+"|"+pos)
				t._handle.css({"left":t._hPos+"px"});
				//t._handle.addCss(t._fix.pos + ':' + t._hPos + 'px');
			}
		},
		_mouseStart: function(event){
			var t = this;
			t._diffPos = event[t._fix.pageAxis] - parseInt(t._handle.css("left").replace("px",""));
			return true;
		},
	...
	$.ui.Mouse = function(element) {
		...
		element.mousedown(t._mouseDown.bind(t)).click(function(event) {
	...
	$.ui.Mouse.prototype = {
		_mouseDown: function(event) {
			...
			jQuery(document).mousemove(t._mouseMoveDelegate).mouseup( t._mouseUpDelegate);
			event.preventDefault();
			return true;
		},
		_mouseMove: function(event) {
			var t = this;
			if (/msie/.test(navigator.userAgent.toLowerCase()) && !event.button) {
		...
		_mouseUp: function(event) {
			...
			jQuery(document).unbind('mousemove', t._mouseMoveDelegate).unbind('mouseup', t._mouseUpDelegate);
			if (t._mouseStarted) {
