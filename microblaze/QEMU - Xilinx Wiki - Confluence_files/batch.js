...
define("confluence/autocomplete-user",["jquery","ajs","document","confluence-rest/confluence-rest","confluence/api/constants"],function(c,k,n,p,q){return function(f){f=f||n.body;var r=function(d){if(!d||!d.result)throw Error("Invalid JSON format");c.each(d.result,function(a,c){c.key=c.username||c.name;"group"===c.type&&(c.title=c.name,c.link=[],c.thumbnailLink={href:k.contextPath()+"/images/icons/avatar_group_48.png"})});var g=[];g.push(d.result);return g};c.each(["user","group","user-or-group"],
function(d,g){c("input.autocomplete-"+g+'[data-autocomplete-user-or-group-bound!\x3d"true"]',f).each(function(){var a=c(this).attr("data-autocomplete-user-or-group-bound","true").attr("autocomplete","off"),d=a.attr("data-max")||10,f=a.attr("data-alignment")||"left",l=a.attr("data-dropdown-target"),e=null,m=a.attr("data-target"),k=a.attr("data-show-unlicensed"),h=m&&c(m);l?e=c(l):(e=c("\x3cdiv\x3e\x3c/div\x3e"),a.after(e));a.attr("data-resize-to-input")&&(e.width(a.outerWidth()),e.addClass("resize-to-input"));
e.addClass("aui-dd-parent autocomplete");"user"===g?a.quicksearch("/rest/api/search",function(){a.trigger("open.autocomplete-user-or-group");a.trigger("open.autocomplete-user")},{makeParams:function(a){return{limit:d,cql:'user.fullname~"'+a+'"'}},dropdownPlacement:function(a){e.append(a)},makeRestMatrixFromData:function(a){if(!a||!a.results)throw Error("Invalid JSON format");c.each(a.results,function(a,b){b.key=b.user.accountId;b.username=b.user.username;b.title=b.user.displayName;b.thumbnailLink=
{href:b.user.profilePicture.path||q.CONTEXT_PATH+"/images/icons/profilepics/default.png"};b.link=[{href:"/people/"+b.user.accountId,type:"text/html",rel:"alternate"}]});var b=[];b.push(a.results);return b},addDropdownData:function(c){if(!c.length){var b=a.attr("data-none-message");b&&c.push([{name:b,className:"no-results",href:"#"}])}return c},ajsDropDownOptions:{alignment:f,displayHandler:function(a){return a.name},selectionHandler:function(t,b){b.find(".search-for").length?(a.trigger("selected.autocomplete-user-or-group",
{searchFor:a.val()}),a.trigger("selected.autocomplete-user",{searchFor:a.val()})):(b.find(".no-results").length||(b=c("span:eq(0)",b).data("properties"),h?(a.val(b.restObj.title),h.val(b.restObj.key)):a.val(b.restObj.key),a.trigger("selected.autocomplete-user-or-group",{content:b.restObj}),a.trigger("selected.autocomplete-user",{content:b.restObj})),this.hide(),t.preventDefault())}}}):a.quicksearch(p.REST.getBaseUrl()+"search/"+g+".json",function(){a.trigger("open.autocomplete-user-or-group")},{makeParams:function(a){return{"max-results":d,
query:a,"show-unlicensed":k}},dropdownPlacement:function(a){e.append(a)},makeRestMatrixFromData:r,addDropdownData:function(c){if(!c.length){var b=a.attr("data-none-message");b&&c.push([{name:b,className:"no-results",href:"#"}])}return c},ajsDropDownOptions:{alignment:f,displayHandler:function(a){return a.name},selectionHandler:function(d,b){b.find(".search-for").length?a.trigger("selected.autocomplete-user-or-group",{searchFor:a.val()}):(b.find(".no-results").length||(b=c("span:eq(0)",b).data("properties"),
h?(a.val(b.restObj.title),h.val(b.restObj.key)):a.val(b.restObj.key),a.trigger("selected.autocomplete-user-or-group",{content:b.restObj})),this.hide(),d.preventDefault())}}})})})}});require("confluence/module-exporter").safeRequire("confluence/autocomplete-user",function(c){Confluence.Binder.autocompleteUserOrGroup=c});
}catch(e){WRMCB(e)};
;
try {
