...
define("confluence-ui-components/js/cql/internal/filterfields/cql-user-filter-field",["underscore","jquery","ajs","confluence/api/constants","confluence-ui-components/js/user-group-select2"],function(d,e,c,b,a){function f(){var g;return{setupInput:function(h){g=h;h.addClass("select2-input autocomplete-multiusergroup");a.bind(h.parent())},setValues:function(m){var h=m.values;if(!h||!h[0]){return}function j(i){return{id:i,text:"Former user",imgSrc:b.CONTEXT_PATH+"/images/icons/profilepics/default.png"}}var o=[];var n=[];for(var k=0;k<h.length;k++){var l=function(i){return e.getJSON(b.CONTEXT_PATH+"/rest/api/user?username="+i).done(function(p){if(p&&p.accountId){n.push({id:p.username,text:p.displayName,imgSrc:p.profilePicture.path})}else{n.push(j(i))}}).fail(function(){n.push(j(i))})};o.push(l(h[k]))}return e.when.apply(e,d.map(o,function(p){var i=e.Deferred();p.always(function(){i.resolve()});return i.promise()})).done(function(){if(n.length){g.auiSelect2("data",n)}})}}}return{build:f}});
}catch(e){WRMCB(e)};
;
try {
