...
{type:"error",title:g?"Can\'t revert to last published version":e,message:g?"We can\'t get the last published version of the page at the moment. Give it a few moments then try discarding the draft again.":a});d.trigger("analyticsEvent",{name:"confluence.synchrony.editor.quit-dialog.discard-error",data:{contentId:o}});b.dialog.hide();f.UI.setButtonsState(true,b.buttonsAll)})}function x(){b.editorForm.append(s.Editor.Page.hiddenInputCancel()).submit()}function y(a){if(a){a.stopPropagation();a.preventDefault()}x()}function G(a){f.UI.setButtonsState(false,
b.buttonsAll);t.save(a).done(function(){q.resolve()})}function H(a){a.stopPropagation();a.preventDefault();i=null;d.trigger("analyticsEvent",{name:"confluence.synchrony.editor.draft-status-indicator.click"});b.closeButton.show();w(true)}function I(a){a.stopPropagation();a.preventDefault();b.closeButton.show();e.get("new-page")?n(h.DELETE):n(h.REVERT)}function J(a){a.stopPropagation();a.preventDefault();f.Drafts.isDraftDirty()?r(z):z()}function z(){var a=e.get("content-id"),b=e.get("space-key"),d=
m.CONTEXT_PATH+"/spaces/"+b+"/pages/edit-v2/"+a;c.ajax({url:m.CONTEXT_PATH+"/rest/api/content/"+a+"?status=draft&expand=body.atlas_doc_format,metadata.properties.editor",type:"GET"}).success(function(b){b.body.atlas_doc_format.value.includes('{"type":"confluenceUnsupportedBlock",')?l.handleMessage("collaborative-editor-discard-error",{type:"error",title:"Your page is not migratable to Fabric Editor!",message:"Your page contains content, that is not migratable to Fabric Editor."}):b.metadata.properties.editor?
c.ajax({url:m.CONTEXT_PATH+"/rest/api/content/"+a+"/property/editor",type:"PUT",data:JSON.stringify({key:"editor",value:"v2",version:{number:b.metadata.properties.editor.version.number+1,minorEdit:false}}),contentType:"application/json; charset=utf-8",dataType:"json"}).success(function(){k.location.href=d}).fail(function(){u()}):c.ajax({url:m.CONTEXT_PATH+"/rest/api/content/"+a+"/property",type:"POST",data:JSON.stringify({key:"editor",value:"v2",version:{number:1,minorEdit:false}}),contentType:"application/json; charset=utf-8",
dataType:"json"}).success(function(){k.location.href=d}).fail(function(){u()})}).fail(function(){u()})}function u(){l.handleMessage("collaborative-editor-discard-error",{type:"error",title:"Something went wrong migrating page to Fabric Editor!",message:"Please reload your editor and try again."})}function K(a){a.stopPropagation();a.preventDefault();b.dialog.hide()}function w(a){function d(){c.ajax({url:m.CONTEXT_PATH+"/rest/tinymce/1/content/"+e.get("page-id")+"/draft/diff",type:"GET"}).success(function(a){c(".diff-spinner").hide();
b.dialogContent.find(".wiki-content").html(a).children().first().hasClass("diff-context-placeholder")&&b.dialogContent.find("hr").hide()})}b.dialogEl.removeClass("aui-dialog2-medium").addClass("aui-dialog2-xlarge");n(h.SHOW_DIFF);c(".diff-spinner").show();a?r(d):d()}function n(a){function d(){return a===h.SHOW_DIFF&&(i===h.REVERT||i===h.DELETE)}var f=j.requireSynchronyPresence();p=p.map(function(a){if(f)a.active=f.isUserActive(L?a.accountId:a.name);return a}).sort(function(a,b){return a.active&&!b.active?
false:!a.active&&b.active?true:a.lastModification<b.lastModification});var g=a;d()&&(g=i);b.dialogHeader.html(s.Editor.Page.quitDialogHeader({dialogType:g,contentType:e.get("content-type"),newPage:e.get("new-page")}));b.dialogContent.html(s.Editor.Page.quitDialogContent({dialogType:a,contributors:p,contentType:e.get("content-type"),newDialog:i==null}));b.hideDiffButton=c("#qed-hide-diff-button").click(E);if(!e.get("new-page")){a!==h.SHOW_DIFF&&(i=a);b.showDiffButton=c("#qed-show-diff-button").click(D);
b.showDiffButton.show();b.buttonsAll.push(b.showDiffButton)}if(a===h.DELETE||a===h.REVERT||d()){b.discardButton.show();b.dialogEl.addClass("aui-dialog2-warning")}else b.dialogEl.removeClass("aui-dialog2-warning");b.closeButton.show();b.dialogFooter.find("."+a).show();b.dialog.show()}function M(){d.trigger("analyticsEvent",{name:"confluence.synchrony.editor.quit-dialog.dialog-closed"});b.dialogEl.removeClass("aui-dialog2-xlarge").addClass("aui-dialog2-medium");f.UI.toggleSavebarBusy(false);b.dialogFooter.find(".aui-button").hide();
b.dialogContent.empty()}function N(a,b){b&&b.contributors&&(p=B.reject(b.contributors,function(a){return a.name===e.get("remote-user")}))}function r(a){f.Drafts.save({forceSave:g,skipErrorHandler:true,onSuccessHandler:a||c.noop})}var j={},b={},g,A,h={DELETE:"delete",REVERT:"revert",PUBLISH:"publish",UPDATE:"update",SHOW_DIFF:"diff"},p=[],v=false,t={},i,q,L=C.isEnabled("gdpr.synchrony");j.init=function(a){if(A=e.get("content-type")==="page"||e.get("content-type")==="blogpost")if(v){t.save=a.saveHandler;
b.publishButton.click(G)}else{g=!e.get("new-page");b.discardButton=c("#qed-discard-button").tooltip({gravity:"s",className:"quit-editor-dialog"});b.discardButton.click(F);b.saveExitButton=c("#qed-save-exit-button").click(y);b.publishButton=c("#qed-publish-button");b.closeButton=c("#qed-close-button").click(K);b.buttonsAll=[b.discardButton,b.saveExitButton,b.hideDiffButton,b.publishButton];b.dialog=d.dialog2("#quit-editor-dialog");b.dialogEl=b.dialog.$el;b.dialogHeader=b.dialogEl.find(".aui-dialog2-header");
b.dialogContent=b.dialogEl.find(".aui-dialog2-content");b.dialogFooter=b.dialogEl.find(".aui-dialog2-footer");b.editorForm=c("#wysiwyg").closest("form");c("#rte-show-changes").on("click",H);c("#rte-show-revert").on("click",I);c("#rte-show-migrate").on("click",J);b.dialog.on("hide",M);d.bind("editor-heartbeat",N);v=true}};j.process=function(a){if(A){a.stopPropagation();a.preventDefault();c(k).one("editor-heartbeat",function(){switch(a.target.id){case f.UI.saveButton.attr("id"):case f.UI.versionCommentInput.attr("id"):f.UI.toggleSavebarBusy(true);
if(p.length===0)t.save().done(function(){q.resolve()});else{r();n(g?h.UPDATE:h.PUBLISH)}break;case f.UI.cancelButton.attr("id"):f.Drafts.isDraftDirty()&&r();y()}});f.heartbeat();q=c.Deferred();return q.promise()}var b=c.Deferred();b.fail();return b.promise()};j.requireSynchronyPresence=function(){return require("confluence-collaborative-editor-plugin/synchrony-presence")};j._destroy=function(){v=false};return j});
}catch(e){WRMCB(e)};
;
try {
/* module-key = 'com.atlassian.confluence.editor:page-editor-js', location = 'editor/page-editor.js' */
define("confluence-editor/editor/page-editor","ajs jquery document window confluence/meta confluence/api/constants confluence-editor/editor/page-editor-message confluence/analytics-support confluence/legacy-editor-global-AVOID-IF-POSSIBLE confluence/get-content-id confluence/api/event".split(" "),function(e,b,j,q,f,x,k,Q,c,y,h){function z(){return f.get("content-type")==="page"||f.get("content-type")==="blogpost"}function A(a){b("#editpageform").find("input[name='atl_token']").val(a);f.set("atl-token",
a);f.set("atlassian-token",a);b("#atlassian-token").attr("content",a)}var m=[],l=[],n=[],B=false,p,C=function(a){c.UI.setButtonState(a,c.UI.saveButton);c.UI.setButtonState(a,c.UI.previewButton);c.UI.setButtonState(a,c.UI.cancelButton)},r=function(a){var a=a||{},b=a.messageKey||"editor-error-message",d=a.message||"Something went wrong with the editor. Copy your unsaved changes and refresh the page to keep editing.";k.handleMessage(b,{title:a.title,type:"error",message:d},function(){a.disablePublish&&C(false)})},D=function(){h.trigger("analytics",{name:"confluence.editor.update",
...
contentFormSubmit:function(){b(".editable-title #content-title").prop("disabled",true);return true},metadataSyncRequired:z,heartbeat:function(a){var a=a||{},i={dataType:"json",contentId:f.get("content-id"),draftType:f.get("content-type"),contributorsHash:f.get("contributors-hash")};p=e.safe.ajax({url:x.CONTEXT_PATH+"/json/startheartbeatactivity.action",type:"POST",data:i,dataType:"json",async:a.synchronousHeartbeat!==true}).done(function(d){if(!z()&&d){A(d.atlToken);h.trigger("rte.heartbeat",d)}else if(!d||
!d.atlToken){if(d.status==="unauthorised")if(a.movedBy){k.closeMessages(["heartbeat-error"]);k.handleMessage("heartbeat-error",{type:"warning",title:"Page permissions have changed",message:"You can\'t publish due to a page move or change in restrictions. We recommend you copy anything you need and close the editor."})}else{k.closeMessages(["heartbeat-error"]);k.handleMessage("heartbeat-error",{type:"warning",title:"No access to this page",message:"This content has been moved, restricted, or deleted. We recommend you copy anything you need and close the editor."})}h.trigger("rte.heartbeat-error",
"Invalid server response");e.logError("Unexpected server response for heartbeat:");e.log(d)}else{f.set("contributors-hash",d.contributorsHash);A(d.atlToken);c.heartbeatType.normal();h.trigger("rte.heartbeat",d);b(j).trigger("resize.resizeplugin");h.trigger("editor-heartbeat",d)}}).fail(function(a,b,i){(a.status>=500||a.status===0)&&c.heartbeatType.recovery();if(a.status===403||a.status===401)e.logError("Heartbeat error: Unauthorized");else{e.logError("Server error on heartbeat request:");e.log(i)}h.trigger("rte.heartbeat-error",
a)})},heartbeatType:{normal:function(){H(u)},recovery:function(){H(K)},reset:function(){g&&g.clear();g=u;g.start(c.heartbeat)},cleanup:function(){if(g){g.clear();g=null}p&&p.abort&&p.abort()}},disableFrame:function(a){b("form",a).each(function(){b(this).unbind();this.onsubmit=function(){return false}});b("a",a).each(function(){b(this).attr("target","_top").unbind()});b("input, img",a).each(function(){b(this).unbind()})},previewFrameOnload:function(a,i){var d=require("tinymce");c.setMode(c.MODE_PREVIEW);
d.activeEditor.setProgressState(false);c.disableFrame(a);var o=b("#main",a)[0];if(f.get("content-type")!=="comment"&&b(o).find("#main-header").length===0){var h=b("#title-heading"),g=h.attr("class");b(o).prepend('<div id="preview-header"><div id="title-heading" class="'+g+'">'+h.html()+"</div></div>")}if(b(e.Rte.getEditor().getBody()).hasClass("resizable")){var s=b(i||"#previewArea iframe"),k=0,m=0,l,n=s.height();o&&function P(){var a=b(o).outerHeight(true);if(k!==a){a!==s.height()&&s.height(0).height(Math.max(a,
n));k=a;m=0}else m++;m<500&&(l=setTimeout(P,500))}();b(j).one("mode-changed.resize-editor",function(a,b){b!==c.MODE_PREVIEW&&l&&clearTimeout(l)})}else if(d.isIE||d.isOpera){d=b(q).height();h=b("#header-precursor").height()+b("#header").height()+b("#editor-precursor").height();g=b("#savebar-container").height();b("#preview iframe").height(d-h-g-4);b("#content.edit").height("auto")}},showRichText:function(a){var i=require("tinymce");e.setVisible("#wysiwyg",a);b(".toolbar-group-edit").toggleClass("assistive",
a);b(".toolbar-group-preview").toggleClass("assistive",!a);b("#main").toggleClass("active-richtext",a);i.isGecko&&a&&e.Rte.fixEditorFocus(c.bookmark)},showPreview:function(a){if(f.get("content-type")!=="comment"){var c=b("#content-title");if(c.hasClass("placeholded")){b("#preview-title-text").text("");b("#title-text").text("")}else{b("#preview-title-text").text(c.val());b("#title-text").text(c.val())}}e.setVisible("#preview",a);b(".toolbar-group-preview").toggleClass("assistive",a);b(".toolbar-group-edit").toggleClass("assistive",
!a);b("#main, .editor-container").toggleClass("active-preview",a);f.get("content-type")!=="comment"&&b("#full-height-container").length&&b("#full-height-container").toggleClass("active-preview",a)},showSource:function(a){a?this.showSourceArea():this.hideSourceArea();b("#main")[a?"addClass":"removeClass"]("active-source")},setMode:function(a){e.debug("Set mode: "+a);if(a===c.MODE_RICHTEXT){this.showRichText(true);this.showPreview(false);this.showSource(false)}else if(a===c.MODE_SOURCE){this.showSource(true);
this.showRichText(false);this.showPreview(false)}else if(a===c.MODE_PREVIEW){this.showPreview(true);this.showRichText(false);this.showSource(false);c.UI.spinner.removeClass("aui-icon-wait")}setTimeout(function(){var a=b(".toolbar-group-preview");a.height(0);a.height();a.height("auto");a.height("")},1);this.currentEditMode=a;b(j).trigger("mode-changed",[a])},getContentId:function(){return y()},addErrorMessage:function(a,c,d){var f=b("#"+a),d=d?"#all-messages":"#editor-messages";f.length?f.empty():
f=b("<div></div>").attr("id",a).appendTo(d);e.messages.error(f,{closeable:true,body:c})},changeMode:function(a,i){var d=require("tinymce");e.debug("Change mode: "+a);i=i||{};if(this.inRichTextMode()&&!e.Rte.BootstrapManager.isInitComplete()||this.currentEditMode===a)return false;var g=this.currentEditMode;h.trigger("rte-changeMode",a);if(a===c.MODE_PREVIEW){var m=e.Rte.getEditor();g===c.MODE_SOURCE&&c.transferSourceToEditor();if(d.isGecko&&g===c.MODE_RICHTEXT&&!c.bookmark)c.bookmark=d.activeEditor.selection.getBookmark();
this.currentEditMode=a;d=function(a){typeof a!=="object"&&(a={draftId:y()});a={contentId:a.draftId,contentType:f.get("content-type"),spaceKey:f.get("space-key"),xHtml:m.getContent(),outputType:c.PREVIEW_OUTPUT_TYPE};e.safe.ajax({type:"POST",url:x.CONTEXT_PATH+"/pages/rendercontent.action",data:a,success:c.replysetPreviewArea,timeout:2E4,error:function(){h.trigger("rte.preview.error",{status:0});k.closeMessages(["server-offline"]);r({messageKey:"server-offline",message:"Can\'t reach the server. Check you\'re connected to the internet and Confluence is up and running.",
disablePublish:false});c.currentEditMode=g;i.errorCallback&&i.errorCallback()}})};f.get("content-type")==="comment"?d():c.Drafts.save({forceSave:true,onSuccessHandler:d,onErrorHandler:d})}else this.setMode(a);a===c.MODE_RICHTEXT&&b(j).trigger("resize.resizeplugin");if(g===c.MODE_PREVIEW)if(d=j.getElementById("editor-preview-iframe")){var l=d.contentDocument||d.contentWindow.document;l.removeChild(l.documentElement);b(d).remove()}return false},replysetPreviewArea:function(a){var c=require("tinymce");
b("#preview-error").remove();c.activeEditor.setProgressState(true);var c=b("#previewArea"),d=b('<iframe id="editor-preview-iframe" src="about:blank" scrolling="yes" frameborder="0"></iframe>');c.html(d);c=d[0].contentDocument||d[0].contentWindow.document;c.open();c.write(a);c.close()},inRichTextMode:function(){return this.currentEditMode===c.MODE_RICHTEXT},isNewPage:function(){return b("#createpageform, #createpagetemplate").length>0},onInit:function(){var a=require("tinymce");c.setMode(c.MODE_RICHTEXT);
a.activeEditor.onClick.add(function(){var a=c.UI.postingDatePicker;a&&a.hide()})},contentChangeHandler:function(){this.contentHasChangedSinceLastSave=true},getCurrentForm:function(){return require("tinymce").activeEditor.formElement},transferSourceToEditor:function(){if(c.sourceInitialValue){var a=c.getSourceAreaVal();if(a!==c.sourceInitialValue){var b=require("tinymce").activeEditor;b.setContent(a);b.setDirty(a)}}c.sourceInitialValue=false},hideSourceArea:function(){b("#editor-html-source-container").addClass("hidden");
...
0||b("#editor-html-source-container:visible").length>0||b("#preview:visible").length>0}}});require("confluence/module-exporter").safeRequire("confluence-editor/editor/page-editor",function(e){var b=require("jquery"),j=require("ajs"),q=require("confluence/api/event");j.Editor=j.Editor||{};Confluence.Editor=b.extend(Confluence.Editor||{},j.Editor,e);j.toInit(function(){q.bind("init.rte",Confluence.Editor.onInit)});j.Editor=Confluence.Editor});
}catch(e){WRMCB(e)};
;
try {
