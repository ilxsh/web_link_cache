define([
    'underscore',
    './computeRTEPluginStyle',
    'application/config'
], function(_, computeRTEPluginStyle, config) {

    // Need to modified blogposts
    var simplified = document.querySelector('.j-design-latest') != null,
        simplifiedPosts = simplified && document.querySelector('.jive-create-post') != null;
    ...
    var getSettings = function(id) {
        ...
        if (id == 'mini-w-quote') {
            ...
            settings.theme_advanced_buttons1 = 'bold,italic,underline,strikethrough,separator,bullist,numlist,separator,jiveimage' + jiveVideoPlugin + 'separator,jivelink,jiveemoticons,jivequote,spellchecker,html';
            delete settings.theme_advanced_buttons2;
            delete settings.theme_advanced_buttons3;
            settings.default_height = 29;
        } else if (id == 'mini') {
            var _jive_image_picker_url = false;
            var _jive_video_picker__url = _jive_video_picker__url;
            settings = getSettings(0);
            settings.theme_advanced_buttons1 = 'bold,italic,underline,strikethrough,separator,bullist,numlist,separator,jiveimage' + jiveVideoPlugin + 'separator,jivelink,jiveemoticons,spellchecker,html';
            delete settings.theme_advanced_buttons2;
            delete settings.theme_advanced_buttons3;
            settings.default_height = 29;
            ...
            var _jive_video_picker__url = false;
            settings = getSettings(0);
            settings.theme_advanced_buttons1 = 'bold,italic,underline,strikethrough,bullist,numlist,html';
            delete settings.theme_advanced_buttons2;
            delete settings.theme_advanced_buttons3;
            settings.default_height = 29;
            // remove jivementions and jiveapps
            settings.plugins = settings.plugins.replace(',jivemention', '');
            settings.plugins = settings.plugins.replace(',jiveapps', '');
        } else if (id == 'stream-narrow') {
            var _jive_image_picker_url = false;
            var _jive_video_picker__url = false;
            settings = getSettings(0);
            settings.theme_advanced_buttons1 = 'bold,italic,underline,strikethrough,bullist,numlist,jiveimage' + jiveVideoPlugin + 'jivelink,jiveemoticons,jivequote,spellchecker';
            delete settings.theme_advanced_buttons2;
            delete settings.theme_advanced_buttons3;
            settings.default_height = 29;
        } else if (id == 'widget') {
            var _jive_image_picker_url = false;
            settings = getSettings(0);
            settings.rte_image_modal_url = CS_BASE_URL + '/rte-image-modal.jspa?fromWidget=true';
            ...
            settings = getSettings('mini');
        } else if (id== 'task') {
            settings = getSettings('mini');
            settings.rte_image_webonly = true;
            settings.rte_image_modal_url = CS_BASE_URL + '/rte-image-modal.jspa?webOnly=true';
        } else if (id == 0) {
            settings = {
                ...
                theme_advanced_buttons1: 'fontselect, fontsizeselect, removeformat, separator,bullist, numlist, ltr, rtl, outdent, indent, separator' + jiveVideoPlugin + 'separator,jivelink,jivetablebutton,extra,spellchecker,html',
                theme_advanced_text_colors: '303030,505050,909090,DADADA,FFFFFF,E23D39,EB7A3D,F1B259,FAF387,7ED529,3A0699,3334CA,2873EE,4FCDDC,5EE697',
                theme_advanced_buttons2: 'bold,italic,underline,strikethrough,forecolor,jivestyle, separator, justifyleft,justifycenter,justifyright,justifyfull, separator,jiveimage,separator,jiveemoticons, jivemacros ',
                ...
                body_class: 'tiny_mce_content jive-widget-formattedtext'
                    + (simplifiedPosts ? ' jive-image-offset' : '')
                    + (simplified ? ' j-rte-latest' : ''),
                theme: 'advanced',
                ...
                content_css: computeStyle(CS_RESOURCE_BASE_URL + '/styles/tiny_mce3/themes/advanced/skins/default/content.css,' + CS_RESOURCE_BASE_URL + '/styles/jive-icons.css'),
                theme_advanced_resize_horizontal: false,
                theme_advanced_resizing: true,
                theme_advanced_grid_width: 5,
                toolbar: 'ltr rtl',
                directionality: (document.querySelector('body[dir="rtl"]') != null ? 'rtl' : 'ltr'),
            ...
            //    var pluginRTESettings = getSettings(0);
        ...
        if (!config.appsAvailable) {
            settings.plugins = _.without(settings.plugins.split(','), 'jiveapps').join(',');
        }
