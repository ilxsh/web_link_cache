...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			if ((/^([\w\-\+\.]+):/i.test(u) && !/^https?:/i.test(u)) || /^\s*#/.test(u)) {
				t.source = u;
				return;
			}
...
tinymce.util.Quirks = function(editor) {
	...
	function bodyHeight() {
		editor.contentStyles.push('body {min-height: 250px}');
    ...
    function createEmptyFirstNode() {
        editor.onFocus.add(function(editor) {
            var bodyText = editor.getBody().textContent;
...
(function(tinymce) {
    ...
    tinymce.create('tinymce.dom.Selection', {
        ...
        getRng : function(w3c) {
            var self = this, selection, rng, elm, win = self.win || self.editor.getWin(), doc = win.document;
        ...
        isForward: function(){
            ...
            try {
                ...
                // Safari 6.0.x can throw exception here
                return true;
            }
        },
        ...
        normalize : function() {
            ...
            function normalizeEndPoint(start) {
                ...
                if (collapsed) {
                    ...
                    // TODO: This conflicts with jiveselection/editor_plugin_src -> make work nicely
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.Button:tinymce.ui.Control', {
		...
		renderHTML : function() {
            ...
            h += '<label class="mceVoiceLabel mceIconOnly j-508-label" id="' + this.id + '_voice">' + s.title + '</span>';
			h += '</label>';
			return h;
		},
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.SplitButton:tinymce.ui.MenuButton', {
		...
		renderHTML : function() {
            ...
            h1 += DOM.createHTML('label', {'class': 'mceVoiceLabel mceIconOnly j-508-label', id: t.id + '_voice'}, s.title);
	...
	tinymce.create('tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			...
			each(is(s.colors, 'array') ? s.colors : s.colors.split(','), function(c) {
                ...
                // add 'div' wrapper to style instead styling 'td', FF doesn't handle css position of table elements well.
                n = DOM.add(n, 'div', {'class': 'mceColorWrapper'});
...
tinymce.create('tinymce.ui.ToolbarGroup:tinymce.ui.Container', {
	renderHTML : function() {
        ...
        h.push('<label id="' + t.id + '_voice" class="mceVoiceLabel j-508-label">' + dom.encode(settings.name) + '</label>');
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		initContentBody : function() {
            ...
            self.selectionUtil = new tinymce.SelectionUtil(self);
          ...
          //Focus events
          self.dom.bind(self.getBody(), "focus", function(){
              self.onFocus.dispatch(self);
          });
          self.dom.bind(self.getBody(), "blur", function(){
              self.onBlur.dispatch(self);
          });
		...
		convertURL : function(url, name, elm) {
			...
			if (!settings.convert_urls || (elm && elm.nodeName == 'LINK') || !url || url.indexOf('file:') === 0)
				return url;
		...
		function handleShortcut(e, execute) {
			if (e.altKey || e.ctrlKey || e.metaKey) {
				each(self.shortcuts, function(shortcut) {
                    ...
                    //Users perfer both ctrl/cmd work on mac, as previously via bug

					if ((shortcut.ctrl != e.ctrlKey && tinymce.isMac && shortcut.ctrl != e.metaKey)
                        || shortcut.alt != e.altKey || shortcut.shift != e.shiftKey)
						return;
	...
	tinymce.UndoManager = function(editor) {
		...
		editor.onKeyDown.add(function(editor, e) {
			...
			if ((keyCode < 16 || keyCode > 20) && keyCode != 224 && keyCode != 91
                && !e.ctrlKey && !e.metaKey && !e.altKey && !self.typing) {
				self.beforeChange();
				self.typing = true;
				self.add();
			}
	...
	tinymce.Formatter = function(ed) {
		...
		function defaultFormats() {
			register({
				...
				removeformat : [
                    // Remove macro alerts
                    {selector : 'pre.jive_macro_alert', remove : 'all', split : true, expand : false, block_expand : true, deep : true },
                    // Default removes
...
(function() {
    ...
    function getIsContainerName(elementName){
        ...
        return function(treePos){
            return treePos && treePos.c && treePos.c.nodeType == 1 && treePos.c.nodeName.toLowerCase() == elementName;
        }
    }
    ...
    function nodeAt(treePos){
        if(treePos && treePos.c && treePos.off >= 0 && treePos.c.childNodes && treePos.c.childNodes.length > treePos.off){
    ...
    //            this.logRange("before adjustForBookmark: ", rng);
            ...
            splitAtEndpoints: function(rng, stopPredicate) {
                ...
                // wrap instead of splitting tables - move markers outside of table if not within the same table cell
                function wrapTable(startMarker, endMarker) {
                    var isStartMarkerInTable =!!$j(startMarker).closest('table').length,
                        isEndMarkerInTable = !!$j(endMarker).closest('table').length;

                    // not inside a table - disregard
                    if(!isStartMarkerInTable && !isEndMarkerInTable)
                        return;

                    // inside the same cell - disregard
                    var $startCell = $j(startMarker).closest('th,td'), $endCell = $j(endMarker).closest('th,td'), table;
                    if($startCell.get(0) === $endCell.get(0))
                        return;

                    if(isStartMarkerInTable) {   // move startMarker before table
                        table = $startCell.closest('table').get(0);
                        table.parentNode.insertBefore(startMarker, table);
                    }
                    if(isEndMarkerInTable) {     // move endMarker after table
                        table = $endCell.closest('table').get(0);
                        dom.insertAfter(endMarker, table);
                    }
                }
                ...
                if(!rng.collapsed){
                    ...
                    wrapTable(startMarker, endMarker);
                }
    ...
    //                this.logRange("Expanded block range: ", ret);
            ...
            dfsNext: function dfsNextFn(treePos, isForward, isRoot){
                var self = this;
                var dom = self.ed.dom;
                function hasNullParent(treePos){
                    return !treePos.c || treePos.c.parentNode == null;
                }
                ...
                function nextRootward(treePos){
                    if((!isRoot ||isRoot(treePos)) || !treePos.c || !treePos.c.parentNode){
                        return null;
                    }
            ...
            safeNormalize: function(rng, n){
                ...
                if(!tinymce.isGecko && !tinymce.isIE9){
                    ...
                    if(this.ed.selection.getSel().rangeCount === 0){
                        //webkit loses the selection if its container disappears, but preserves range semantics
	...
	tinymce.EnterKey = function(editor) {
		...
		function handleEnterKey(evt) {
			...
			if (isCaretAtStartOrEndOfBlock()) {
                ...
                if (tinymce.isIE && !tinymce.isIE11 && parentBlock.childNodes.length > 0
                    && parentBlock.childNodes[parentBlock.childNodes.length-1].nodeName == "BR") {
                    dom.remove(parentBlock.childNodes[parentBlock.childNodes.length-1]);
                }
