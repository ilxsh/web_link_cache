...
define([
    ...
    jive.rte.Macro = function(shortname, url, macrotag, settingsHuh, displayHuh, paramSets, params, enabled, button) {
        ...
        if (typeof(jive.rte.plugin[shortname]) != 'undefined') {
            custommacro = new jive.rte.plugin[shortname](shortname, url, macrotag, settingsHuh, displayHuh, paramSets,
                params, enabled, button);
        }
        ...
        this.caresAboutChangeTo = function(rte, ele, selNode) {
            if (custommacro != null && custommacro.caresAboutChangeTo) return custommacro.caresAboutChangeTo(rte, ele,
                selNode);
            return false;
        };
        ...
        this.refresh = function(rte, ele) {
            ...
            if (ele.getAttribute('jivemacro') == this.getName()) {
                if (this.getMacroType().toLowerCase() == 'inline') {
                    ...
                    if (ele.src == '') {
                        var src = window.CS_RESOURCE_BASE_URL +
                            '/images/tiny_mce3/plugins/jiveemoticons/images/spacer.gif';
